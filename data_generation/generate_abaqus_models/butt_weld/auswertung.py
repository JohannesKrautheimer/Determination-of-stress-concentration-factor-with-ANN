...
## Das Skript durchläuft alle .odb-files in einem Ordner und schreibt die extrahierten Ergebnisse in die Datei models_list_results.json (zusammen mit den Daten aus models_list.json)
...
odb_dir_path = "finished_abaqus_files_and_param_fields/param_field_0/odb-files/2022"
os.chdir(odb_dir_path)
models_list_path = "C:/Projekte/datengenerierung/finished_abaqus_files_and_param_fields/param_field_0/models_list.json"
result_file_name = "models_list_results.json"
result_file_path = "C:/Projekte/datengenerierung/finished_abaqus_files_and_param_fields/param_field_0/result-files/" + result_file_name
...
for key in odb_dict:
    ...
    ## EVAL_POINT
    p_nodeLabel, p_value, p_coordinates, p_tensor = getMaxInNodeset(odb, odb.rootAssembly.instances['BUTT_WELD-1'].nodeSets['EVAL_POINT'], invariant=abqC.MAX_PRINCIPAL)
    ## EVAL_RADIUS
    r_nodeLabel, r_value, r_coordinates, r_tensor = getMaxInNodeset(odb, odb.rootAssembly.instances['BUTT_WELD-1'].nodeSets['EVAL_RADIUS'], invariant=abqC.MAX_PRINCIPAL)
    odb_dict[key].update(
        {
            "results": {
                "eval_radius": {
